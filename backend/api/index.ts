...
// Middleware d'authentification webhook (optionnel)
const authenticateWebhook = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (!apiKey) {
    return next(); // Pas de clé configurée, on autorise
  }
...
// Route webhook Discord
app.post('/api/discord/webhook', authenticateWebhook, async (req, res) => {
  try {
    const { gameId, username, action, amount, description, category } = req.body;
    ...
    let user = null;
    if (gameId) {
      user = await prisma.user.findUnique({ where: { gameId } });
    }
    if (!user && username) {
    ...
    const transactionType = (action === 'deposit' || action === 'income') ? 'INCOME' : 'EXPENSE';
    const transactionDescription = description || 
      (transactionType === 'INCOME' ? `Entrée d'argent depuis Discord` : `Sortie d'argent depuis Discord`);
    ...
    const transaction = await prisma.transaction.create({
      ...
      include: {
        user: {
          select: { id: true, name: true, username: true, gameId: true },
        },
      },
    });
...
// Route webhook GTA RP
app.post('/api/gtarp/webhook', authenticateWebhook, async (req, res) => {
  try {
    const { gameId, action, amount, description, category } = req.body;
    
    if (!gameId || !action || !amount) {
      return res.status(400).json({ error: 'gameId, action et amount requis' });
    }
    
    if (action !== 'deposit' && action !== 'withdraw') {
      return res.status(400).json({ error: 'action doit être "deposit" ou "withdraw"' });
    }
    
    const user = await prisma.user.findUnique({ where: { gameId } });
    
    if (!user) {
      return res.status(404).json({ error: 'Aucun utilisateur trouvé avec cet ID de jeu', gameId });
    }
    
    const transactionType = action === 'deposit' ? 'INCOME' : 'EXPENSE';
    const transactionDescription = description || 
      (action === 'deposit' ? `Dépôt depuis coffre GTA RP` : `Retrait vers coffre GTA RP`);
    ...
    console.error('Erreur webhook GTA RP:', error);
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
